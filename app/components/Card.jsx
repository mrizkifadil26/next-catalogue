...
export default function Card({ movie }) {
    return (
        <div className="rounded-xl overflow-hidden shadow-lg bg-gray-900/80 border border-gray-800 hover:shadow-2xl hover:scale-105 transition-transform duration-300 flex flex-col">

            {/* Poster */}
            <div className="relative w-full h-72">
                <img
                    src={movie.poster ? `https://image.tmdb.org/t/p/w342${movie.poster}` : "https://via.placeholder.com/342x513?text=No+Image"}
                    alt={movie.title}
            ...
            <div className="p-4 flex flex-col flex-1 justify-between space-y-3">
                ...
                <div className="space-y-1">
                    <h3 className="font-semibold text-base md:text-lg text-gray-100 line-clamp-2">
                        ...
                        {movie.original_title && movie.original_title !== movie.title && (
                            <span className="text-gray-400 text-sm">({movie.original_title})</span>
                        )}
                    </h3>
                ...
                {/* Language left / Tags right */}
                <div className="flex justify-between mt-2">
                    {/* Language */}
                    {movie.language && (
                        <span className={`text-xs px-3 py-1 rounded-md text-white ${langColor}`}>
